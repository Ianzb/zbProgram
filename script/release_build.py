...
def compile_exe():
    ...
    if USE_NUITKA:
        cmd = [sys.executable, "-m", "nuitka", "--clang", "--msvc=latest", "--assume-yes-for-downloads", "--show-progress", f"--force-stdout-spec={zb.joinPath(zb.TEMP_PATH, NAME + ".log")}", f"--force-stderr-spec={zb.joinPath(zb.TEMP_PATH, NAME + ".log")}",
