...
Port notes (nju-xk addon):
  - Verbatim port of D:\\Code\\NJU-xk-helper\\lib\\captcha.py (481 lines).
  - Model paths are relative to this file (captcha/models/).
  - numpy / onnxruntime / PIL are imported lazily inside _build_solver()
    so this module can be imported even when those deps are missing;
    a missing dep surfaces as a catchable exception at call time.
  - _NORM_PRESETS is built lazily on first use for the same reason.
"""
...
UPPER_ONNX_PATH = os.path.join(_MODULE_DIR, "models", "upper_model.onnx")
TITLE_ONNX_PATH = os.path.join(_MODULE_DIR, "models", "title_model.onnx")
...
def _get_norm_presets() -> Dict[str, dict]:
    """Lazily build normalization presets (avoids numpy at import time)."""
    global _NORM_PRESETS
    if _NORM_PRESETS is None:
        import numpy as np
        _NORM_PRESETS = {
            "imagenet": {
...
def _fg_mask_faint(arr: np.ndarray, sat_thr: float = 0.03) -> np.ndarray:
    """
    Foreground mask for faint/desaturated chars (fallback pass only).

    Why this exists (todo 8, capture #25): the standard _fg_mask drops any
    pixel with R>165, G>205, B>225 as "light background". Some captchas
    render one char in a very light greenish-gray color (e.g. RGB(231,240,238),
    saturation ~0.04) that passes all three thresholds and is wrongly removed,
    leaving only 3 of 4 regions. Such faint chars are distinguishable from the
    blue gradient background (which has B>G) by having B<=G, so refining
    light_bg with (B>G) keeps them while still removing the background. A low
    saturation threshold then recovers the char. Only used by _segment_upper's
    fallback pass, so images that already segment cleanly are unaffected.
    """
    r = arr[..., 0].astype(np.float32)
    g = arr[..., 1].astype(np.float32)
    b = arr[..., 2].astype(np.float32)
    ...
    minc = np.minimum(np.minimum(r, g), b)
    sat = (maxc - minc) / (maxc + 1e-6)
    light_bg = (arr[..., 0] > 165) & (arr[..., 1] > 205) & (arr[..., 2] > 225) & (b > g)
    return (sat > sat_thr) & (~light_bg)
...
def _segment_upper(img: Image.Image) -> List[dict]:
    ...
    # Faint-char fallback (todo 8, capture #25): some captchas render one char
    # in a very light desaturated color that the standard light_bg filter drops
    # (see _fg_mask_faint). Only reached when the ladder above found <4 regions,
    # so images that already segment cleanly (e.g. capture #32) are unaffected.
    mask = _fg_mask_faint(arr)
    regions = _connected_components(mask, min_area=8)
    regions = _merge_nearby_regions(regions, dist_thresh=20)
    if len(regions) >= 4:
        return regions[:8]
...
def _preprocess(img: Image.Image, input_size: int, norm: str) -> np.ndarray:
    ...
    presets = _get_norm_presets()
    preset = presets.get(norm, presets["imagenet"])
...
def _build_solver():
    # Lazy imports: numpy / onnxruntime / PIL are only required at call time.
    # Importing them here (as module globals) keeps this module importable
    # even when the deps are missing; a missing dep raises ImportError here,
    # which _get_solver caches and solve_captcha_from_base64 catches.
    global np, ort, Image
    import numpy as np
...
def solve_captcha_from_base64(img_gif_b64_body: str) -> Optional[List[Point]]:
    ...
    try:
        ...
        logging.warning("[captcha] solve failed: %s", e)
        return None
